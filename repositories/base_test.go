...
func (bs *BaseTestSuite) setupMigrations(ctx context.Context, networks []string, postgresqlIp string) error {

	g := StdoutLogConsumer{}

	postgresHost := fmt.Sprintf("postgres://ant:secret@%s/service_ledger?sslmode=disable", net.JoinHostPort(postgresqlIp, "5432"))
	...
	cRequest := testcontainers.ContainerRequest{
		...
		ConfigModifier: func(config *container.Config) {
			config.Env = []string{
				"LOG_LEVEL=debug",
				"DO_MIGRATION=true",
				fmt.Sprintf("DATABASE_URL=%s", postgresHost),
			}
		},
		Networks:   networks,
