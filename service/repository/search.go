...
func NewSearchEngine(service *frame.Service, namespace string) (*SearchEngine, utility.ApplicationLedgerError) {
	if namespace != SearchNamespaceAccounts &&
		...
		return nil, utility.ErrorSearchNamespaceUnknown
	}
...
func NewSearchRawQuery(_ context.Context, q string) (*SearchRawQuery, utility.ApplicationLedgerError) {
	var rawQuery *SearchRawQuery
	err := json.Unmarshal([]byte(q), &rawQuery)
	if err != nil {
		return nil, utility.ErrorSearchQueryHasInvalidFormart
	}
	...
	for _, item := range checkList {
		if !hasValidKeys(item) {
			return nil, utility.ErrorSearchQueryHasInvalidKeys
		}
	}
	return rawQuery, nil
}
