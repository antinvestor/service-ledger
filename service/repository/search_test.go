...
func (ss *SearchSuite) SetupSuite() {
	...
	ss.accDB = repository.NewAccountRepository(ss.service)
	ss.txnDB = repository.NewTransactionRepository(ss.service, ss.accDB)
	ss.ledgerDB = repository.NewLedgerRepository(ss.service)

	lg, err := ss.ledgerDB.Create(ss.ctx, &models2.Ledger{Type: models2.LEDGER_TYPE_ASSET})
	if err != nil {
